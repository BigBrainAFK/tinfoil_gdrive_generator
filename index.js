...
async function listDriveFiles(driveId = null) {
	...
	if (!flags.root && !auto) rootfolder = await question('Whats the root folder id: ');
	if (!flags.root && auto) {
	...
	const python = require('child_process').spawn('python', ['encrypt.py', 'output\\index.json', 'shop\\index.json']);
	...
	if (driveId) {
		...
		if (!driveAnswer && !auto) driveAnswer = await question(`Write to ${selectedDrive}? [y/n]:`);
		if (!driveAnswer && auto) {
